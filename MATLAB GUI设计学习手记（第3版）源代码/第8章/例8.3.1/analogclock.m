...
t = timer('Period', 1, 'TimerFcn', {@timeUpdate, hAxes}, 'BusyMode', 'queue',...
    'ExecutionMode', 'fixedRate');  %创建定时器
start(t);  %启动定时器

function timeUpdate(obj, ~, hAxes)
%% 循环更新指针位置
if ishandle(hAxes)
    %% 删除指针
    delete(findobj(hAxes, 'Type', 'hggroup'));   %删除3个指针，方便下次更新指针
    %% 更新当前时间
    ...
    arrow(hAxes, pi/2 - argHour, 'cyan', 0.5); %绘制时针
    argMin = (min + sec/60) * pi / 30;    %计算分针的弧度值
    arrow(hAxes, pi/2 - argMin, 'red', 0.8);    %绘制分针
    argSec = sec * pi / 30;               %计算秒针的弧度值
    arrow(hAxes, pi/2 - argSec);   %绘制秒针
