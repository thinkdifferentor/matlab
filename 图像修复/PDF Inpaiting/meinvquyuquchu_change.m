...
clear,clc
figure(1);
fillimg=imread('lena_inp_2.bmp'); 
subplot(1,2,1);imshow(fillimg); title('原图像');
I=double(fillimg)/255;
sz=[size(fillimg,1),size(fillimg,2)];
fillregion=fillimg(:,:,1)==255;%fillRegion为待修复点为1(白色)，已存在点为0
...
subplot(1,2,2);imshow(I2); title('修复图');
