...
void bestexemplarhelper1(const int mm, const int nn, const int m, const int n, 
...
/* 
 *  bestexemplarhelper1(mm,nn,m,n,img,Ip,toFill,sourceRegion); */
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) 
{
  ...
  /* Setup the output */
  plhs[0] = mxCreateDoubleMatrix(4,1,mxREAL);//创建数组
  best = mxGetPr(plhs[0]);//返回数组的实部指针；
  best[0]=best[1]=best[2]=best[3]=0.0;

  /* Do the actual work */
  bestexemplarhelper1(mm,nn,m,n,img,Ip,toFill,sourceRegion,best);//对best处理，best为引用类型！
}
