...
% img=imread('C:\Users\Administrator\Desktop\zwg\');
% imshow(img);
% % figure(1)
% % imshow(fillRegion,[]);
% 
...
% % 
% % fillImg = LL;%imread('img2.bmp');
 fillImg = imread('bw2.png');
%  
% % fillRegion=zeros(size(LL));
...
%     LL(rows,cols) = ind2img(ind(rows,cols),LL);
%      LH(rows,cols) = ind2img(ind(rows,cols),LH);
%      HH(rows,cols) = ind2img(ind(rows,cols),HH);
%      HL(rows,cols) = ind2img(ind(rows,cols),HL);
end
inpaintedImg= origImg%idwt2(LL,LH,HL,HH,'db1');
imshow(mat2gray(inpaintedImg))
