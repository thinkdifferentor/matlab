clc
clear
bmp=imread('C:\Users\Administrator\Desktop\zwg\barb.png');

imshow(uint8((double(bmp))));
tic;
...
while any(fillRegion(:))
    ...
    origImg(rows,cols) = ind2img(ind(rows,cols),origImg);%ind(rows,cols)表示待修复块的坐标范围，该范围已被最优匹配块的值所替代
     for i=1:length(y)
         if(i==1)
             y{i}(rows,cols)=ind2img(ind(rows,cols),y{i});
         else
             for j=1:length(y{i})
                 y{i}{j}(rows,cols)=ind2img(ind(rows,cols),y{i}{j}); 
             end
         end
     end         
