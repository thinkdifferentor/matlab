...
%%图片读入(下标从1开始)
for index=0:Num-1
    Image{1,index+1}=imread(['附件2\',sprintf('%03d',index),'.bmp'],'bmp');
...
%定义白色像素图片
Empty=cell(1,1);
Empty{1,1}=uint8(255*ones(Row,Colum));
...
for i=2:Num
    ...
    %合成一张图片并显示
    for j=1:i
        ImageSorted{1,j}=Image{Sorted(1,j)};
    end
    
    for k=i+1:Num
       ImageSorted{1,k}=Empty{1,1}; 
    end
    
    Timg=cell2mat(ImageSorted);
...
%保存文件
imwrite(Img,'附件2\Solution_English.bmp','bmp');
